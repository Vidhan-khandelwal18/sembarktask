...
function ProductCard({ product }: Props) {
  ...
  return (
    <article className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-200 flex flex-col">
      ...
      >
        <img src={imgSrc} alt={product.title} className="w-full h-48 object-cover bg-slate-100" />
        <div className="p-3 flex flex-col gap-1 flex-1">
          <h2 className="text-sm font-semibold text-slate-800 line-clamp-2 m-0">{product.title}</h2>
